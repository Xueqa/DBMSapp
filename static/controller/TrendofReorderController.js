...
async function postfunc(ctx,next){
    ...
    if(param.method=='byMonth'){
        result=await funcs.reorderTrendsByMonth(param.aisle_name,param.start_date,param.end_date);
        title='Reorder Trends by Month';
        ...
        result=await funcs.reorderTrendsByDay(param.aisle_name,param.start_date,param.end_date);
        title='Reorder Trends by Day';
        xAxis=new Array(result.length);
        data=new Array(result.length);
        for(var i=0;i<result.length;i++){
            data[i]=result[i][0];
