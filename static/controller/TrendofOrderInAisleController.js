...
async function getfunc(ctx, next) {
    ...
    //console.log(aisle);
    await ctx.render('TrendofOrderInAisle',{
        aisle:aisle
    });
...
async function postfunc(ctx,next){
    ...
    if(param.method=='byMonth'){
        result=await funcs.orderTrendsByMonth(param.start_date,param.end_date);
        ...
        result=await funcs.orderTrendsByDay(param.start_date,param.end_date);
